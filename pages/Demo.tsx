...
import { CheckCircle, Copy, Download, ArrowLeft, Package } from 'lucide-react';
...
const Demo: React.FC = () => {
    ...
    const transaction = {
        ref_id: 'TRX-123456789',
        created_at: new Date().toISOString(),
        buyer_email: 'demo@example.com',
        product_title: 'Netflix Premium 1 Bulan (2 Item)',
        price: 25000,
        quantity: 2,
        fee: 500,
        unique_code: 123,
        get_balance: 50000,
        // Simulation of 2 items joined by delimiter
        stock_content: "Email: user1@netflix.com\nPass: pass123\nExpired: 20 Feb 2024\n---\nEmail: user2@netflix.com\nPass: secure456\nExpired: 20 Feb 2024"
    };
    ...
    // Parse Stock Content
    const stocks = transaction.stock_content.split('\n---\n');
                    ...
                    <div className="p-6 sm:p-8 space-y-8 print:p-0 print:px-8">
                        ...
                        <div className="space-y-3 pt-4 border-t border-slate-100 dark:border-slate-800 print:border-slate-200">
                            <div className="flex justify-between text-sm text-slate-600 dark:text-slate-400 print:text-slate-600">
                                <span>Total Harga (x{transaction.quantity})</span>
                                <span>{formatPrice(transaction.get_balance)}</span>
                            </div>
                            <div className="flex justify-between text-sm text-slate-600 dark:text-slate-400 print:text-slate-600">
                            ...
                            <div className="flex justify-between text-lg font-bold pt-3 border-t border-dashed border-slate-200 dark:border-slate-700 print:border-slate-300 text-slate-900 dark:text-white print:text-black">
                                ...
                                <span className="text-indigo-600 dark:text-indigo-400 print:text-black">
                                    {formatPrice(transaction.get_balance + transaction.fee + transaction.unique_code)}
                                </span>
                            </div>
                        </div>
